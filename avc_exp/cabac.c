...
const opt_enum_t av_pic_type[] = {
    {"N",   AV_PICTURE_TYPE_NONE  },
    {"I",   AV_PICTURE_TYPE_I     },
    {"P",   AV_PICTURE_TYPE_P     },
    {"B",   AV_PICTURE_TYPE_B     },
    {"S",   AV_PICTURE_TYPE_S     },
...
const int n_av_pic_type = ARRAY_SIZE(av_pic_type);
...
/**
 * verify: cabac_state[ctxIdx] = 2 * pStateIdx[ctxIdx] + valMPS
 * cabac_state[] defined in ffmpeg H264SliceContext.cabac_state[1024]
 * pStateIdx defined in <9.3.1.1, H264 spec (2005,03)>
 */
void ff_h264_init_cabac_states(int cabac_state[1024], int slice_qp, 
                               int slice_type, int cabac_init_idc)
{
    int i;
    const int8_t (*tab)[2];

    if (slice_type == AV_PICTURE_TYPE_I) tab = cabac_context_init_I;
    else                                 tab = cabac_context_init_PB[cabac_init_idc];

    /* calculate pre-state */
    for( i= 0; i < 1024; i++ ) {
        int pre1 = 2*(((tab[i][0] * slice_qp) >>4 ) + tab[i][1]) - 127;
        int pre = pre1;
        pre^= pre>>31;
        if(pre > 124)
            pre= 124 + (pre&1);
        ...
        /* <ITU-T Recommendation H.264 (2005,03)> */
        /* 9.3.1.1 Initialisation process for context variables */
        int preCtxState, pStateIdx, valMPS;
        preCtxState = clip(((tab[i][0]*clip(slice_qp, 0, 51)) >> 4) + tab[i][1], 1, 126);
        if( preCtxState <= 63 ) {
        ...
        xprint("ctxIdx=%4d, valMPS=%d, spec:%3d -> %3d, ff:%4d -> %4d\n", 
                i, valMPS, preCtxState, pStateIdx, pre1, pre);
    }

    return;
}

typedef struct cabac_opt{
    int slice_qp;
    int slice_type;
    int cabac_idc;
}cabac_opt_t;

#define CABAC_OPT_M(member) offsetof(cabac_opt_t, member)
cmdl_opt_t cabac_opt[] = 
{
    { 0, "h,help",              0, cmdl_parse_help,         0,      0,  },
    { 0, "xl,xlevel,xall,xnon", 0, cmdl_parse_xlevel,       0,      0,     "config log level"},
    { 0, "qp",                  1, cmdl_parse_int,  offsetof(cabac_opt_t, slice_qp),   "26",  "slice qp"},
    { 0, "t,pic-type",          1, cmdl_parse_int,  CABAC_OPT_M(slice_type), "%P",  "slice type"},
    { 0, "idc",                 1, cmdl_parse_int,  CABAC_OPT_M(cabac_idc),  "0",  "cabac_init_idc"},
};
const int n_cabac_opt = ARRAY_SIZE(cabac_opt);
...
int cabac_test(int argc, char **argv)
{
    ...
    cmdl_set_enum(n_cabac_opt, cabac_opt, "pic-type", n_av_pic_type, av_pic_type); 
    cmdl_iter_t iter = cmdl_iter_init(argc, argv, 0);
    int r = cmdl_parse(&iter, &cfg, n_cabac_opt, cabac_opt);
    if (r == CMDL_RET_HELP) {
        return cmdl_help(&iter, 0, n_cabac_opt, cabac_opt);
    } else if (r < 0) {
        xerr("cmdl_parse() failed, ret=%d\n", r);
        return 1;
    }
    
    int cabac_state[1024] = {0};
    ff_h264_init_cabac_states(cabac_state, cfg.slice_qp, cfg.slice_type, cfg.cabac_idc);
